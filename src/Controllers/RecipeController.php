...
use App\Domain\Recommendation\{RecommendationContext, CalorieDeficitStrategy, MuscleGainStrategy, MaintenanceStrategy};
...
class RecipeController extends Controller {
    ...
    public function suggest() {
        ...
        $strategy = match (strtolower($user->objetivo ?? '')) {
    'emagrecimento','deficit','perda de peso' => new CalorieDeficitStrategy(),
    'ganho muscular','hipertrofia' => new MuscleGainStrategy(),
    default => new MaintenanceStrategy(),
};
$ctx = new RecommendationContext($strategy);
$recommended = $ctx->recommend($filtered, $user);
$this->view('recipes/suggest.php',[ 'user'=>$user, 'items'=>$recommended ]);
    }
}
